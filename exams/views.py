...
from .models import (
    Student, CourseStudent, Question, Answer, Exam, ExamSubject,
    ExamResult, StudentAnswer, AnswerOrder, StudentImport,
)
from .utils import compute_result_stats
...
def check_answer_correctness(student_answer):
    ...
    if question.question_type in ('open', 'text'):
        ...
        if matched:
            ...
            if exam_subject:
                student_answer.points_earned = exam_subject.points_for_difficulty(question.difficulty)
        else:
            student_answer.points_earned = 0
        return
